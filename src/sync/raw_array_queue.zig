...
pub fn ConcurrentArrayQueue(comptime T: type) type {
    return ConcurrentArrayQueueImpl(T, usize, std.atomic.Atomic(usize));
}
        ...
        pub fn try_push(queue: *Queue, item: T) bool {
            ...
            while (true) {
                ...
                if (2 *% queue.turn(head) == sturn) {
                    if (queue.head.tryCompareAndSwap(head, head +% 1, .Monotonic, .Monotonic)) |new_head| {
                        // CAS failure
                        head = new_head;
                    } else {
        ...
        pub fn try_pop(queue: *Queue) ?T {
            ...
            while (true) {
                ...
                if (2 *% queue.turn(tail) +% 1 == sturn) {
                    if (queue.tail.tryCompareAndSwap(tail, tail +% 1, .Monotonic, .Monotonic)) |new_tail| {
                        // CAS failure
                        tail = new_tail;
                    } else {
